...
# -*- coding: utf-8 -*-
"""
Alpha Competition brand pricing test — all Alpha Competition rows on the upload sheet.
...
Preview all Alpha Competition rows:
  python test_alpha_competition_pricing.py

Only rows missing «cost price»:
  python test_alpha_competition_pricing.py --missing-cost

Apply to Google Sheet + Shopify:
  python test_alpha_competition_pricing.py --apply

One SKU (# optional):
  python test_alpha_competition_pricing.py --sku "#ALP-123"
  python test_alpha_competition_pricing.py --apply --sku "#ALP-123"

Limit (smoke test):
  python test_alpha_competition_pricing.py --limit 5
...
DEFAULT_BRAND = "Alpha Competition"
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Alpha Competition brand pricing test — scrape and optionally update all rows.",
    )
    parser.add_argument(
        "--brand",
    ...
    parser.add_argument(
        "--output",
        default="",
        help="CSV summary (default: alpha_competition_pricing_test_YYYYMMDD_HHMMSS.csv)",
    )
    args = parser.parse_args()
    do_apply = args.apply

    print("=" * 70)
    print(f"Alpha Competition brand pricing test — {args.brand} (upload sheet)")
    print("=" * 70)
    if do_apply:
    ...
    print(
        "Scrape: distribution cost+retail (paginated), motorsport availability + retail fallback"
    )
    ...
    if args.sku:
        ...
        if not variants:
            print("No rows matched --sku (try with/without #).")
            return 1
    ...
    if not out_path:
        ...
        out_path = os.path.join(SCRIPT_DIR, f"alpha_competition_pricing_test_{stamp}.csv")
    ...
    if do_apply:
        ...
        print("  Re-run with: python test_alpha_competition_pricing.py --apply")
        print("  Missing cost only: python test_alpha_competition_pricing.py --apply --missing-cost")
    print("=" * 70)
    return 0
